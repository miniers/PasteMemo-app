...
- **可复用片段** -- 任意剪贴板内容都能保存为片段，支持标题、分组、标签，并可在主窗口和快捷粘贴中快速复用。
...
- **从历史保存** -- 任意剪贴板历史都可以保存为可复用片段，并自定义标题。
- **片段管理** -- 可在主窗口编辑标题、内容、类型、置顶状态、分组和标签。
- **统一搜索** -- 在一个输入框里同时搜索片段标题、内容、分组和标签。
- **快捷粘贴联动** -- 片段可直接出现在侧边栏、快捷粘贴面板和统一搜索结果中，全程键盘操作。
- **批量操作** -- 可多选片段后统一分组、增删标签、置顶、复制或删除。
- **使用统计** -- 记录最近使用时间与使用次数，让高频片段更容易浮现。

### 快捷粘贴面板

- **全局快捷键** -- 在任何应用中按下 Cmd+Shift+V（可自定义，支持 F1-F12）打开快捷粘贴面板。
- **键盘导航** -- Cmd+1 到 Cmd+9 直接粘贴，方向键或 Ctrl+P / Ctrl+N 浏览历史，左/右方向键切换内容类型，回车粘贴。
- **片段范围 + 统一搜索** -- 可在历史记录和片段之间快速切换，也可以在统一搜索里同时搜索两者并在结果分区间跳转。
...
| 左/右方向键 | 切换内容类型 |
| Cmd+左 / Cmd+右 | 在历史记录和片段之间切换 |
| Cmd+上 / Cmd+下 | 在统一搜索中跳转历史/片段分区 |
| Cmd+S | 切换历史或片段排序 |
| 回车 | 粘贴选中项 |
| Shift+回车 | 粘贴并回车 |
| Cmd+K | 打开快捷操作 |
